...
Implement the `error.tsx` and `global-error.tsx` components for a Next.js application, along with a centralized API error response utility. Errors should be logged for debugging but never expose internal details to the client. Include the error response format for API routes.
