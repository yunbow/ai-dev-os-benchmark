...
Implement a `POST /api/v1/auth/login` route handler in Next.js App Router. The endpoint accepts email and password, authenticates the user against the database using NextAuth.js v5, and returns a session. Include proper cookie configuration and input handling.
