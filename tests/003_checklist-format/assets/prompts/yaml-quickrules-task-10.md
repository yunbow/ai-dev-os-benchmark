...
# Extracted for benchmark evaluation (Version C: YAML frontmatter + Quick Rules)
rules:
...
> **Quick Rules (28 items)**
>
> 1. Default deny — grant only necessary permissions
> 2. Separate responsibilities across layers (Client -> Server -> DB)
> 3. Require Zod validation for all input data
> 4. Never include secrets in code
> 5. CSRF prevention — SameSite=Lax or stricter on cookies
> 6. Origin / Referer checks in API Routes
> 7. Clickjacking prevention — X-Frame-Options: DENY and CSP frame-ancestors 'none'
> 8. Implement RBAC/ABAC authorization at Route Handler layer
> 9. IDOR prevention — verify resource ownership on every mutating request
> 10. IP-based rate limiting via Edge Middleware
> 11. Webhook signature verification before processing
> 12. Webhook replay attack prevention — idempotency check with event ID
> 13. Webhook timestamp validation — reject events older than 5 minutes
> 14. CSP nonce per request — no unsafe-inline, use strict-dynamic
> 15. Webhook certificate URL SSRF prevention — domain allowlist
> 16. HTML injection prevention — escape user data in email templates
> 17. Error page information leakage prevention — no stack traces in responses
> 18. Session management — max 5 concurrent sessions, suspicious login detection
> 19. Minimum DB privileges — app user has no DDL permissions
> 20. Application-layer encryption for personal data (AES-256)
> 21. Encryption key management via dedicated secret service with rotation
> 22. Separate env vars for Development / Preview / Production
> 23. HTTPS only — Secure attribute on cookies, wss:// for WebSockets
> 24. Minimize OAuth scopes; short-lived access tokens with encrypted refresh tokens
> 25. Cookie-based sessions with Secure / HttpOnly / SameSite flags
> 26. GDPR compliance — support data deletion and data export requests
> 27. Vulnerability response SLA — Critical within 24h, High within 7 days
> 28. Security monitoring — log auth/authz failures, alert on anomalies
...
Implement a session management system that limits concurrent sessions to 5 per user, detects suspicious logins from new countries, and sends notification emails. Include a `GET /api/v1/users/me/sessions` endpoint that lists active sessions and a `DELETE /api/v1/users/me/sessions/:id` endpoint for remote logout. Also implement a user data export endpoint for GDPR compliance.
