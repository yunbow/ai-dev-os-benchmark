...
Set up the environment configuration for a Next.js application deployed on Vercel. Create the `.env.example` file, a Zod-validated env config module, and document how secrets should be managed across development, preview, and production environments. Include OAuth provider configuration with appropriate scopes.
