...
Implement a `POST /api/webhooks/payment` route handler that receives payment webhook events. The handler should verify the webhook is legitimate, process the payment status update, and update the user's subscription accordingly. Include all necessary security checks.
