...
Implement the `middleware.ts` file for a Next.js application. It should add security headers to all responses, handle CSRF protection for API routes, and generate CSP nonces for inline scripts. The middleware should also handle the `/admin` path protection.
