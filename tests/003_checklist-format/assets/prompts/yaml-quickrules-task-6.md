...
Implement an admin dashboard API route at `GET /api/v1/admin/users` that lists all users with pagination. Only users with the ADMIN role should be able to access this endpoint. The route should be protected at the middleware level with IP restrictions and at the handler level with role checks.
