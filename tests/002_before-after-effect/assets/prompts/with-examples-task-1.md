...
Implement a Server Action for updating a user's profile in a Next.js 15 App Router application.

Requirements:
- Users can update their own name and email
- Use Prisma for database access (User model with id, name, email, hashedPassword fields)
- Use NextAuth v5 for authentication (auth() function returns session)
- Use Zod for input validation
- Return a structured result (not throw exceptions)
