...
Implement a complete password reset flow for a Next.js 15 application.

Requirements:
- Server Action to request a password reset (takes email)
- Generates a single-use reset token with 1-hour expiry
- Sends a reset email with a link
- Server Action to reset password (takes token + new password)
- Validates token, checks expiry, hashes new password, invalidates token

Tech stack: Next.js 15 (App Router), TypeScript (strict), Prisma, bcrypt, nodemailer

Please generate: both Server Actions, the email function, and relevant Prisma schema additions.
