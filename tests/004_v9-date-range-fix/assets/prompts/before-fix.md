Follow these validation guidelines when implementing:
...
Implement a "Create Task" Server Action for a Next.js 15 App Router application.

Requirements:
- Fields: title (required, max 200 chars), description (optional, max 2000 chars), status (TODO/IN_PROGRESS/DONE), priority (LOW/MEDIUM/HIGH), dueDate (optional), categoryId (optional)
- Use Zod for input validation
- Use Prisma for database access
- Return ActionResult pattern (not throw exceptions)
- dueDate should be a valid future date when provided

Tech stack: Next.js 15 (App Router), TypeScript (strict), Prisma, Zod

Please generate the complete Server Action file and the Zod schema file.
