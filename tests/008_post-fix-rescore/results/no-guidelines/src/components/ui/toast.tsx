...
function dispatch(action: ToastState | { type: "remove"; id: string }) {
  if ("type" in action && action.type === "remove") {
    toasts = toasts.filter((t) => t.id !== action.id);
  } else {
    const toast = action as ToastState;
...
export function useToast() {
  ...
  return { toasts: state, toast, dismiss: (id: string) => dispatch({ type: "remove", id }) };
}
...
export function Toaster() {
  ...
  return (
    <ToastProvider>
      {toastList.map(({ id, title, description, variant, action }) => (
